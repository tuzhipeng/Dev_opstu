...
选项：
    cd          进入当前用户的家目录
    cd -        进入上次所在位置，比如开始在root目录下，再到Japan目录，执行cd-就会到root目录下，再执行cd-就会到Japan目录下
    cd ..       进入上一级目录
    注意一下相对路径与绝对路径！
```
### 2.4 删除目录或文件：rm
...
选项：
    ...
    -f          表示强制。所以一般我们搭配使用，即rm -rf，但删除时一定要看清楚，删了就恢复不了了
```
### 2.5 复制目录或文件：cp
```
命令英文原意：copy
...
1、ln  [原文件] 路径 【是否改名】
  ...
  #硬链接特征
    拥有相同的i节点和存储block块，（在格式化时，第一将分区划分为等大小的block数据块，该数据块通常为4KB。
    第二建立分区表，并为每个文件建立一个相关数据。当用户搜索某个文件时，会先到分区表中查找文件所对应的block存储位置，
    接着到相应位置取出，并返回给用户。）
    
    硬链接与原文件拥有相同的i节点。因此，可以看做是同一个文件。当删除原文件或硬链接文件的任何一文件，不影响文件索引操作。
